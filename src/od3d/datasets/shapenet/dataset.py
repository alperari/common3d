...
from od3d.datasets.shapenet.enum import (MAP_CATEGORIES_SHAPENET_V2_ID_TO_CATEGORY,
                                         ...
                                         SHAPENET_CATEGORIES, MAP_CATEGORIES_OD3D_TO_SHAPENET,
                                         SHAPENET_SUBSETS, LIST_CATEGORIES_SHAPENET_CANOC)
...
from od3d.datasets.frame import (OD3D_FRAME_MODALITIES, OD3D_Frame, OD3D_FrameMeshMixin, OD3D_CamProj4x4ObjMixin,
                                 OD3D_FrameRGBMaskMixin, OD3D_FrameMaskMixin, OD3D_FrameRGBMixin, OD3D_FrameDepthMixin,
                                 OD3D_FrameDepthMaskMixin, OD3D_FrameCategoryMixin, OD3D_FrameSequenceMixin,
                                 OD3D_FrameSizeMixin, OD3D_MeshTypeMixin, OD3D_FramePCLMixin, OD3D_FrameMeshMixin,
                                 OD3D_FrameRaysCenter3dMixin, OD3D_FrameTformObjMixin, OD3D_FrameSubsetMixin,
                                 OD3D_FrameMeta)

from od3d.datasets.sequence_meta import (OD3D_SequenceMeta, OD3D_SequenceMetaCategoryMixin, OD3D_SequenceMetaMeshMixin,
                                         OD3D_SequenceMetaPCLMixin)
from od3d.datasets.sequence import (OD3D_SequenceMeshMixin, OD3D_SequenceCategoryMixin,
                                    OD3D_Sequence)

from od3d.cv.geometry.transform import transf3d_broadcast
from od3d.cv.geometry.objects3d.meshes import Meshes
from od3d.datasets.frame import OD3D_MaskTypeMixin, OD3D_CamTform4x4ObjTypeMixin,  OD3D_DepthTypeMixin
...
@dataclass
class ShapeNet_SequenceMeta(OD3D_SequenceMetaPCLMixin, OD3D_SequenceMetaMeshMixin, OD3D_SequenceMetaCategoryMixin,
    OD3D_FrameMetaSubsetMixin, OD3D_SequenceMeta):
...
class ShapeNet(OD3D_SequenceDataset):
    ...
    def extract_meta(config: DictConfig):
        ...
        subsets_rendering = [
            SHAPENET_SUBSETS.R2N2_V2_ICO_UNI32_THETA_UNI1, SHAPENET_SUBSETS.R2N2NOTXT_V2_ICO_UNI16_THETA_UNI1,
            SHAPENET_SUBSETS.R2N2NOTXT_V2_ICO_UNI32_THETA_UNI1, SHAPENET_SUBSETS.R2N2NOTXT_V2_ICO_UNI64_THETA_UNI1,
            SHAPENET_SUBSETS.R2N2NOTXT_V2_ICO_UNI32_THETA_UNI2, SHAPENET_SUBSETS.R2N2NOTXT_V2_ICO_UNI16_THETA_UNI4,
        ...
        for subset in subsets:
            if subset in subsets_rendering:
                ...
                import re
                match = re.match(r"([a-z0-9]+)_v([0-9]+)_([a-z0-9]+)_([a-z]+)([0-9]+)_theta_([a-z]+)([0-9]+)", subset, re.I)
                if match and len(match.groups()) == 7:
                    subset_type, version, viewpoints_type, viewpoints_sample_type, viewpoints_count, theta_sample_type, theta_count = match.groups()
                ...
                for category_shp in LIST_CATEGORIES_SHAPENET_CANOC:
                    ...
                    if path_r2n2_category.exists():
                        sequences_ids = [path.name for path in list(path_r2n2_category.iterdir())]
                    else:
                        sequences_ids = [path.name for path in list(path_raw.joinpath(f"{category_id}").iterdir())]

                    for sequence_id in sequences_ids:

                        if sequences_count == sequences_count_max_per_category:
                            logger.info(f"reached maximum count of sequences {sequences_count_max_per_category}")
                            break

                        logger.info(f"extract meta sequence {sequence_id}")
                        frame_fpath_pcl = path_raw.joinpath(f"pcl/{subset}/{category}/{sequence_id}.ply")
                        frame_fpath_mesh_orig = path_raw.joinpath(f"{category_id}/{sequence_id}/models/model_normalized.obj")
                        ...
                        if not frame_fpath_mesh_orig.exists():
                            logger.info(f"skip sequence {sequence_id} in {category} because does not exist in v2.")
                            continue
                        ...
                        if sequence_meta.get_fpath(path_meta=path_meta).exists():
                            logger.info(f"already extracted sequence {sequence_id} in {category}")
                        ...
                        if subset_type == 'r2n2notxt':
                            ...
                            _mesh_trimesh.visual = trimesh.visual.color.ColorVisuals(_mesh_trimesh, vertex_colors=grey_color)

                        logger.info(f"sequence {sequence_id} in {category}, {len(_mesh_trimesh.vertices)} vertices")
                        # _mesh_trimesh.show()
                        ...
                        if len(_mesh_trimesh.vertices) > 0:
                            ...
                            logger.info(f"skip sequence {sequence_id} in {category} because could not convert to single mesh.")
                        ...
                        from od3d.cv.io import write_image, write_mask_image, write_depth_image, write_pts3d_with_colors_and_normals
                        ...
                        if viewpoints_type.startswith('icotraj') or viewpoints_type.startswith('icorealtraj'):
                            ...
                            cams_tform4x4_obj = get_ico_cam_tform4x4_obj_for_viewpoints_count(
                                viewpoints_count=viewpoints_count, radius=2.5, theta_count=theta_count,
                        ...
                        logger.info(f"sequence {sequence_id} in {category}, {len(mesh.verts)} vertices")
                        ...
                        mods = mesh.render(cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cam_intr4x4,
                                           ...
                                           broadcast_batch_and_cams=True, rgb_bg=[0., 0., 0.], rgb_diffusion_alpha=0.3)
                        # rgbs = mods['rgb'][0]
                        masks = mods['mask'][0]
                        ...
                        for i, cam_tform4x4_obj in enumerate(cams_tform4x4_obj):
                            ...
                            frame_fpath_rgb = path_raw.joinpath(f"rgb/{subset}/{category}/{sequence_id}/{frame_name}.png")
                            frame_fpath_mask = path_raw.joinpath(f"mask/{subset}/{category}/{sequence_id}/{frame_name}.png")
                            frame_fpath_depth = path_raw.joinpath(f"depth/{subset}/{category}/{sequence_id}/{frame_name}.png")
                            frame_fpath_depth_mask = path_raw.joinpath(f"depth_mask/{subset}/{category}/{sequence_id}/{frame_name}.png")


                            rgb, depth = render_trimesh_to_tensor(mesh_trimesh=_mesh_trimesh, cam_tform4x4_obj=cam_tform4x4_obj,
                                                           cam_intr4x4=cam_intr4x4, H=H, W=W, rgb_bg=[0.8, 0.8, 0.8])
                            depth_mask = depth > 0.

                            _pts3d = depth2pts3d_grid(depth=depth.to(device), cam_intr4x4=cam_intr4x4).permute(1, 2, 0)[depth_mask[0]]
                            ...
                            frame_meta = ShapeNet_FrameMeta(name=frame_name,
                                                            ...
                                                            rfpath_depth_mask=frame_fpath_depth_mask.relative_to(path_raw),
                                                            # rfpath_mesh=frame_fpath_mesh.relative_to(path_raw),
                                                            category=category,
                        ...
                        pts3d_ids, pts3d = fps(pts3d=pts3d, K=1024, fill=True)
                        write_pts3d_with_colors_and_normals(pts3d, pts3d_colors=None, pts3d_normals=None, fpath=frame_fpath_pcl)
        ...
        if SHAPENET_SUBSETS.R2N2 in subsets:
            ...
            for category_id in categories_r2n2_id:
                ...
                for sequence_id in sequences_ids:
                    logger.info(f"extract meta sequence {sequence_id}")
                    path_r2n2_sequence = path_r2n2_category.joinpath(f"{sequence_id}/rendering")
                    path_renderings_txt = path_r2n2_sequence.joinpath("renderings.txt")
                    path_renderings_meta_txt = path_r2n2_sequence.joinpath("rendering_metadata.txt")
                    ...
                    import torch
                    from od3d.cv.geometry.objects3d.meshes import Meshes
                    frames_metas_strs = read_str_from_file(fpath=path_renderings_meta_txt).split('\n')
                    frames_metas_cams = torch.Tensor([[float(number) for number in frame_meta_str.split(" ")] for frame_meta_str in frames_metas_strs])
                    # N x 5 {yaw, pitch, 0, radius, fov}
                    # N x 5 {azimuth, elevation, 0, depth_ratio, 25}
                    MAX_CAMERA_DIST = 1.75
                    device = 'cuda'

                    frame_fpath_mesh = path_raw.joinpath(f"ShapeNetCore.v1/{category_id}/{sequence_id}/model.obj")
                    ...
                    for f, frame_name in enumerate(frames_names):
                        ...
                        import math
                        # v1
                        azimuth = torch.Tensor([-frame_meta_cam[0] / 180 * math.pi - math.pi / 2])
                        ...
                        depth_ratio = torch.Tensor([frame_meta_cam[3]]) * MAX_CAMERA_DIST
                        ...
                        from od3d.cv.visual.show import DEFAULT_CAM_TFORM_OBJ, OBJ_TFORM_OBJ_SHAPENET
                        from od3d.cv.geometry.transform import tform4x4, transf4x4_from_spherical

                        cams_tform4x4_obj = transf4x4_from_spherical(azim=azimuth,
                                                                     elev=elevation,
                                                                     theta=theta,
                                                                     dist=depth_ratio).to(device)


                        cams_tform4x4_obj = tform4x4(cams_tform4x4_obj, OBJ_TFORM_OBJ_SHAPENET.clone().to(device)[None,])
                        ...
                        cams_intr4x4 = get_default_camera_intrinsics_from_img_size(fov_x=fov,
    ...
    def setup(config: DictConfig):
        ...
        from od3d.datasets.shapenet.enum import MAP_CATEGORIES_SHAPENET_V2_ID_TO_CATEGORY
        splits = read_json(path_raw.joinpath("ShapeNetRendering/pix2mesh_splits_val05.json"))
        for split in splits.keys():
            ...
            for category_id in splits[split].keys():
                ...
                for sequence_id in category_split_dict[category].keys():
                    category_split_dict[category][sequence_id] = [f"{int(n):02}" for n in category_split_dict[category][sequence_id]]
        ...
        for category_id in categories_ids:
            if path_raw.joinpath(category_id).exists():
                logger.info(f'already there, category: {MAP_CATEGORIES_SHAPENET_V2_ID_TO_CATEGORY[category_id]}, '
                            f'{category_id}')
            else:
                # bicycle
                # table, 04379243
                logger.info(f'get category: {MAP_CATEGORIES_SHAPENET_V2_ID_TO_CATEGORY[category_id]}, {category_id}')
                # $HF_TOKEN
                hf_token = "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"
