...
def names(
    ...
    # logger.info(f"found {df.keys()} runs")

    abls_cols = ablation.split(",")
    abl_col_cat = "categories/cross/pascal3d"
    abl_col_ref = "co3d_refs"
    abls_cols.remove(abl_col_cat)
    ...
    abls_cols_unique = df[abls_cols].drop_duplicates().reset_index(drop=True)
    for u in range(len(abls_cols_unique)):
        abl_col_unique = abls_cols_unique.iloc[u]
        df_partial = df.copy()
...
def multiple(
    ...
    if metrics is not None:
        if metrics == "co3dv1":
            metrics = "test/co3dv1_zsp_10s/pose/acc_pi6,test/co3dv1_zsp_10s/pose/acc_pi12,test/co3dv1_zsp_10s/pose/acc_pi18,test/co3dv1_zsp_10s/iou_pose_pred,test/co3dv1_zsp_10s/iou_pose_gt,test/co3dv1_zsp_10s/geo"
        elif metrics == "pascal3d":
            metrics = "test/pascal3d_test/pose/acc_pi6,test/pascal3d_test/pose/acc_pi18,test/pascal3d_test/kpts2d_acc,test/pascal3d_test/iou_amodal_pose_pred,test/pascal3d_test/iou_amodal_pose_gt,test/pascal3d_test/iou_pose_pred,test/pascal3d_test/iou_pose_gt"
        elif metrics == "objectnet3d":
            metrics = "test/objectnet3d_test/pose/acc_pi6,test/objectnet3d_test/pose/acc_pi18,test/objectnet3d_test/iou_amodal_pose_pred,test/objectnet3d_test/iou_amodal_pose_gt,test/objectnet3d_test/iou_pose_pred,test/objectnet3d_test/iou_pose_gt"
        elif metrics == "spair71k":
            metrics = "test/spair71k/kpts2d_acc/pck01,test/spair71k/kpts2d_acc/pck005,test/spair71k/kpts2d_acc/pck001"
    ...
    if summary_cols is not None:
        ...
        for metric in metrics:
            ...
            if df_metrics is None:
                ...
                df_metrics = pd.concat([df_metrics, df_metric], axis=1)
            # logger.info(tabulate(df_metric, headers='keys', tablefmt='tsv', floatfmt=f".{digits}f", showindex=False)) # latex
            # logger.info(
            #     re.sub(
            #         r"[^\S\r\n]+",
