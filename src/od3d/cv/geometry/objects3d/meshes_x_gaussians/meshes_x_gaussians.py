...
class Meshes_x_Gaussians(Meshes):
    def __init__(
        ...
        if gs_opacity_requires_grad:
            ...
            self.verts_gs_opacity = 1.

        self.gs_top_k = gs_top_k
        if gs_scale_requires_grad:
            self.verts_gs_scale = torch.nn.Parameter(gs_scale * torch.ones((self.verts_count, 3)).to(**factory_kwargs),
                                                     requires_grad=gs_scale_requires_grad)
        else:
            self.verts_gs_scale = gs_scale #  * torch.ones((self.verts_count, 3)).to(**factory_kwargs)
    ...
    def get_stacked_with_mesh_ids(self, tensor_packed, mesh_ids=None):
        ...
        for mesh_id in mesh_ids:
            tensors_padded.append(self.get_tensor_verts_with_pad(
                tensor_packed[self.verts_counts_acc_from_0[mesh_id] : self.verts_counts_acc_from_0[mesh_id + 1]],
                mesh_id=mesh_id,
            ))
    ...
    def render_batch(
        ...
        **kwargs
        ...
        from od3d.cv.render.gaussians_splats_v2 import rasterize_gaussians, select_and_blend_gaussians

        pts3d = self.get_stacked_with_mesh_ids(tensor_packed=self.verts, mesh_ids=objects_ids).clone().to(device=device)

        if isinstance(self.verts_gs_scale, torch.Tensor):
            pts3d_scale = self.get_stacked_with_mesh_ids(tensor_packed=self.verts_gs_scale,
                                                         mesh_ids=objects_ids).clone().to(device=device)
        else:
            pts3d_scale=self.verts_gs_scale
        if isinstance(self.verts_gs_rotation, torch.Tensor):
            pts3d_rotation = self.get_stacked_with_mesh_ids(tensor_packed=self.verts_gs_rotation,
                                                         mesh_ids=objects_ids).clone().to(device=device)
        else:
            pts3d_rotation =self.verts_gs_rotation

        if isinstance(self.verts_gs_opacity, torch.Tensor):
            pts3d_opacity = self.get_stacked_with_mesh_ids(tensor_packed=self.verts_gs_opacity,
                                                         mesh_ids=objects_ids).clone().to(device=device)
        else:
        ...
        pts3d_mask = self.get_verts_stacked_mask_with_mesh_ids(mesh_ids=objects_ids).clone().to(device=device)

        gs_id, px_to_gs_opacity, gs_depth, gs_normal = rasterize_gaussians(
            cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4, imgs_size=imgs_sizes, pts3d=pts3d,
            pts3d_mask=pts3d_mask, pts3d_size=pts3d_scale, pts3d_opacity=pts3d_opacity, pts3d_rotation=pts3d_rotation,
            topK=self.gs_top_k,
        )
            ...
            if modality == PROJECT_MODALITIES.MASK:
                ...
                if modality == PROJECT_MODALITIES.ONEHOT:
                    verts_one_hot_from_faces = self.get_labels_onehot(
                        smooth=False, objects_ids=objects_ids, add_other_objects=add_other_objects,
                        add_clutter=add_clutter, device=device)[:, 0]

                elif modality == PROJECT_MODALITIES.ONEHOT_SMOOTH:
                    verts_one_hot_from_faces = self.get_labels_onehot(
                        smooth=True, objects_ids=objects_ids, add_other_objects=add_other_objects,
                        add_clutter=add_clutter, device=device)[:, 0]

                elif modality == PROJECT_MODALITIES.ONEHOT_COARSE:
                    verts_one_hot_from_faces = self.get_labels_onehot(
                        coarse=True, smooth=False, objects_ids=objects_ids, add_other_objects=add_other_objects,
                        add_clutter=add_clutter, device=device)[:, 0]
                if add_clutter:
                    feat_bg = (
                ...
                mod2d_rendered = select_and_blend_gaussians(gs_id, px_to_gs_opacity, verts_one_hot_from_faces, feat_bg=feat_bg)

            elif modality == PROJECT_MODALITIES.DEPTH:
                mod2d_rendered = select_and_blend_gaussians(gs_id, px_to_gs_opacity, gs_depth)
                ...
                for b in range(B):
                    verts_vsbl_mask[b, gs_id[b, px_to_gs_opacity[b].mean(dim=0).mean(dim=1)[b] > 0].unique()] = 1
                ...
                gs_feats = self.get_feats_stacked_with_mesh_ids(mesh_ids=objects_ids).to(device)
                ...
                mod2d_rendered = select_and_blend_gaussians(gs_id, px_to_gs_opacity, gs_feats,
                                                            feat_bg=feat_bg)

            elif modality == PROJECT_MODALITIES.PT3D_NCDS:
                gs_feats = self.get_verts_ncds_stacked_with_mesh_ids(mesh_ids=objects_ids).to(device)
