...
from od3d.cv.io import write_image, write_depth_image
...
from od3d.datasets.omni6dpose.frame import Omni6DPose_Frame, Omni6DPose_FrameMeta
from od3d.datasets.omni6dpose.sequence import Omni6DPose_Sequence, Omni6DPose_SequenceMeta
...
class Omni6DPose(OD3D_SequenceDataset):
    ...
    def __init__(
        ...
        if not dict_nested_sequences_filter_cat_w_name and not dict_nested_frames_filter_cat_w_name:
    ...
    def preprocess(self, config_preprocess: DictConfig):
        ...
        for key in config_preprocess.keys():
            if key == "objcentric" and config_preprocess.objcentric.get("enabled", False):
                override = config_preprocess.objcentric.get("override", False)
                remove_previous = config_preprocess.objcentric.get("remove_previous", False)
    ...
    def preprocess_mesh(self, remove_previous=False, override=False):
        ...
        for batch in iter(tqdm(dataloader)):
            ...
            fpaths_meshs = frame.get_fpaths_meshs(mesh_type=OD3D_MESH_TYPES.ALPHA500) # self.mesh_type
    ...
    def preprocess_objcentric(self, remove_previous=False, override=False):
        ...
        if not override:
            ...
            for frames_subdir, frames_names in tqdm(dict_nested_frames_unrolled.items()):
                for frame_name in frames_names:
                    if (not self.path_preprocess.joinpath(f"bboxs/{frames_subdir}/{frame_name}/bboxs.pt").exists() or
                            not self.path_preprocess.joinpath(f"rgb/{frames_subdir}/{frame_name}.png").exists() or
                            not self.path_preprocess.joinpath(f"pxl_cat_id/{frames_subdir}/{frame_name}.png").exists() or
                            not self.path_preprocess.joinpath(f"depth/{frames_subdir}/{frame_name}.png").exists()):
                        if frames_subdir not in dict_nested_frames_unrolled_filtered:
                            dict_nested_frames_unrolled_filtered[frames_subdir] = []
                        dict_nested_frames_unrolled_filtered[frames_subdir] += [frame_name]
            dict_nested_frames_filtered = rollup_flattened_dict(dict_nested_frames_unrolled_filtered)
            self = self.get_subset_with_dict_nested_frames(dict_nested_frames=dict_nested_frames_filtered)
        ...
        for batch in iter(tqdm(dataloader)):
            ...
            from od3d.cv.visual.show import show_scene, show_img, render_trimesh_to_tensor, show_imgs
            obj_size = batch.mesh.verts.max() * 2

            mask = batch.mesh.render(cams_tform4x4_obj=batch.cam_tform4x4_obj,
                                     ...
                                     obj_tform4x4_objs=torch.stack(batch.obj_tform4x4_objs).to(device=device),
                                     zfar=10*obj_size)
            ...
            for i, frame_name_unique in enumerate(batch.name_unique):
                ...
                import trimesh
                _mesh_trimesh = trimesh.load(frame.get_fpaths_meshs(mesh_type=OD3D_MESH_TYPES.META)[0], force='mesh')
                ...
                rgb, depth = render_trimesh_to_tensor(mesh_trimesh=_mesh_trimesh,
                                                      ...
                                                      cam_intr4x4=batch.cam_intr4x4[i], H=H, W=W,
                                                      rgb_bg=[0.8, 0.8, 0.8], zfar=obj_size * 10)
                ...
                if frame.fpath_pxl_cat_id.exists() and not override:
                    logger.info(f"pxl_cat_id exists at {frame.fpath_pxl_cat_id}, skipping...")
                else:
                    from od3d.datasets.omni6dpose.enum import MAP_CATEGORIES_OMNI6DPOSE_TO_ID
                    dataset_specific_id = MAP_CATEGORIES_OMNI6DPOSE_TO_ID[frame.categories[0]]
                    write_image(img=(dataset_specific_id * (mask[i] > 0)).to(dtype=torch.uint8), path=frame.fpath_pxl_cat_id)
    ...
    def preprocess_bboxs(self, remove_previous=False, override=False):
        ...
        if remove_previous:
            ...
            subdirs = ['meta/frames', 'meta/sequences', 'rgb', 'depth', 'pxl_cat_id', 'bboxs', 'mask', 'mask_amodal']
                ...
                for subdir in subdirs:
                    if subdir == 'bboxs':
                        ...
                        for bbox_subset in bboxs_subsets:
                            ...
                            if self.path_preprocess.joinpath(subdir, f"{subset}_{bbox_subset}").exists():
                                shutil.rmtree(self.path_preprocess.joinpath(subdir, f"{subset}_{bbox_subset}"))
        ...
        if not override:
            ...
            dict_nested_frames_unrolled_filtered = {}
            for frames_subdir, frames_names in tqdm(dict_nested_frames_unrolled.items()):
                if 'bbox' in frames_subdir: # no preprocessing required for these subsets
                    continue
                for frame_name in frames_names:
                    frames_subset = frames_subdir.split('/')[0]
                    frames_seq = frames_subdir.split('/')[1]

                    if ((not self.path_preprocess.joinpath(f"meta/sequences/{frames_subset}_bbox_mask_amodal/{frames_seq}_{frame_name}.yaml").exists()) or
                            (not self.path_preprocess.joinpath(f"meta/sequences/{frames_subset}_bbox_mask_modal/{frames_seq}_{frame_name}.yaml").exists())):
                        if frames_subdir not in dict_nested_frames_unrolled_filtered:
                            dict_nested_frames_unrolled_filtered[frames_subdir] = []
                        dict_nested_frames_unrolled_filtered[frames_subdir] += [frame_name]
            dict_nested_frames_filtered = rollup_flattened_dict(dict_nested_frames_unrolled_filtered)
        ...
        for batch in iter(tqdm(dataloader)):
            ...
            from od3d.cv.geometry.objects3d.meshes import PROJECT_MODALITIES
            mask_amodal = batch.mesh.render(cams_tform4x4_obj=batch.cam_tform4x4_obj,
                                     cams_intr4x4=batch.cam_intr4x4,
                                     imgs_sizes=batch.size,
                                     objects_ids=batch.mesh_id_in_batch[0],
                                     modalities=PROJECT_MODALITIES.MASK,
            ...
            for bbox_subset in bboxs_subsets:
                ...
                for bbox_id in range(len(bboxs)):
                    ...
                    rgb, cam_crop_tform_cam = crop_with_bbox(
                        img=batch.rgb[0], bbox=bbox, H_out=H_out, W_out=W_out
                    )
                    #from od3d.cv.visual.show import show_img, show_imgs
                    #show_imgs([batch.rgb[0], rgb])

                    pxl_cat_id, _ = crop_with_bbox(
                        img=batch.pxl_cat_id[0], bbox=bbox, mode='nearest_v2', H_out=H_out, W_out=W_out
                    )

                    bbox_mask, _ = crop_with_bbox(
                        img=mask[:, bbox_id], bbox=bbox, H_out=H_out, W_out=W_out
                    )

                    bbox_mask_amodal, _ = crop_with_bbox(
                        img=mask_amodal[bbox_id, :, 0], bbox=bbox, H_out=H_out, W_out=W_out
                    )
                    ...
                    depth, _ = crop_with_bbox(
                        img=batch.depth[0, None], bbox=bbox, H_out=H_out, W_out=W_out
                    )
                    ...
                    if frame_fpath_mask_amodal.exists() and not override:
                        logger.info(f"mask amodal exists at {frame_fpath_mask_amodal}, skipping...")
                    else:
                        write_image(img=bbox_mask_amodal, path=frame_fpath_mask_amodal)

                    if frame_fpath_pxl_cat_id.exists() and not override:
                        logger.info(f"pxl_cat_id exists at {frame_fpath_pxl_cat_id}, skipping...")
                    else:
                        from od3d.datasets.omni6dpose.enum import MAP_CATEGORIES_OMNI6DPOSE_TO_ID
                        ...
                        dataset_specific_id = MAP_CATEGORIES_OMNI6DPOSE_TO_ID[frame.meta.categories[bbox_id]]
                        write_image(img=(dataset_specific_id * (bbox_mask > 0)).to(dtype=torch.uint8),
                    ...
                    frame_meta = Omni6DPose_FrameMeta(
                        ...
                        subset=subset,
                        rfpath_rgb=frame_fpath_rgb.relative_to(self.path_preprocess), # frame.meta.rfpath_rgb,
                        rfpath_pxl_cat_id=frame_fpath_pxl_cat_id.relative_to(self.path_preprocess),
                        rfpath_depth=frame_fpath_depth.relative_to(self.path_preprocess),
                        rfpath_depth_mask=frame_fpath_depth_mask.relative_to(self.path_preprocess),
                        rfpaths_meshs=[fpath.relative_to(self.path_raw) for fpath in fpaths_meshs],
                        l_size=l_size,
                        ...
                        l_obj_tform4x4_objs=frame.meta.l_obj_tform4x4_objs[bbox_id:bbox_id+1],
                        l_objs_name=frame.meta.l_objs_name[bbox_id:bbox_id+1],
                        l_objs_valid=frame.meta.l_objs_valid[bbox_id:bbox_id+1],
                    )

                    if frame_meta.get_fpath(path_meta=self.path_meta).exists() and not override:
                        logger.info(f"already extracted frame {frame_name} in scene {sequence_name} in subset {subset}")
                    else:
                        frame_meta.save(path_meta=self.path_meta)

                    if bbox_id == len(bboxs) -1:
                        seq_meta = Omni6DPose_SequenceMeta(name=sequence_name, subset=subset)
                        if seq_meta.get_fpath(path_meta=self.path_meta).exists() and not override:
                            logger.info(f"already extracted sequence {sequence_name} in subset {subset}")
                        else:
    ...
    def extract_meta(config: DictConfig):
        ...
        for subset in subsets:
            ...
            if subset == OMNI6DPOSE_SUBSETS.TEST_REAL:
                ...
                scenes_names[subset] = [scene_path.stem for scene_path in scenes_paths[subset]]
            elif (subset == OMNI6DPOSE_SUBSETS.TEST_IKEA or subset == OMNI6DPOSE_SUBSETS.TEST_MATTERPORT3D or
                  subset == OMNI6DPOSE_SUBSETS.TEST_SCANNETPP or subset == OMNI6DPOSE_SUBSETS.TRAIN_IKEA or
                  subset == OMNI6DPOSE_SUBSETS.TRAIN_MATTERPORT3D or subset == OMNI6DPOSE_SUBSETS.TRAIN_SCANNETPP):
                ...
                for patch_id in sope_patch_ids:
                    path_patch_subset = path_raw.joinpath('SOPE', patch_id, subset_rpath)
                    if path_patch_subset.exists():
                        scenes_paths[subset] += list(path_patch_subset.iterdir())
                        scenes_names[subset] += [f"{patch_id}_{scene_path.stem}" for scene_path in scenes_paths[subset]]

            for s, scene_path in enumerate(tqdm(scenes_paths[subset])):
                frames_ids = [fpath.stem.split("_")[0] for fpath in filter(lambda p: Path(p).stem.endswith("_color"), scene_path.iterdir())]

                seq_meta = Omni6DPose_SequenceMeta(name=scenes_names[subset][s], subset=subset)
                if seq_meta.get_fpath(path_meta=path_meta).exists() and not config.extract_meta.override:
                    logger.info(f"already extracted sequence {scenes_names[subset][s]} in subset {subset}")
                else:
                ...
                for frame_id in tqdm(frames_ids):
                    ...
                    from od3d.cv.geometry.transform import cam_intr_to_4x4, transf4x4_from_rot3x3_and_transl3, cam_intr4x4_downsample
                    ...
                    img_size = torch.Tensor(
                        [meta['camera']['intrinsics']['height'], meta['camera']['intrinsics']['width']])
                    ...
                    downsamplerate_H = meta['camera']['intrinsics']['height'] / rgb.shape[-2]
                    downsamplerate_W = meta['camera']['intrinsics']['width'] / rgb.shape[-1]
                    ...
                    cam_intr4x4 = cam_intr_to_4x4(
                        fx=meta['camera']['intrinsics']['fx'], fy=meta['camera']['intrinsics']['fy'],
                        cx=meta['camera']['intrinsics']['cx'], cy=meta['camera']['intrinsics']['cy'])


                    cam_intr4x4, img_size = cam_intr4x4_downsample(cams_intr4x4=cam_intr4x4, imgs_sizes=img_size,
                                                                   down_sample_rate=downsamplerate)

                    l_size = img_size.tolist()
                    l_cam_intr4x4 = cam_intr4x4.tolist()
                    from od3d.cv.geometry.transform import transf4x4_from_rot4_and_transl3, inv_tform4x4, \
                        tform4x4_broadcast
                    world_tform4x4_cam = transf4x4_from_rot4_and_transl3(rot4=meta['camera']['quaternion'],
                    ...
                    cam_tform4x4_objs = torch.stack([transf4x4_from_rot4_and_transl3(
                        rot4=obj['quaternion_wxyz'], transl3=obj['translation'])
                        for obj in meta['objects'].values()], dim=0)


                    rfpath_scene_mesh = meta['camera']['scene_obj_path']
                    if subset == OMNI6DPOSE_SUBSETS.TEST_REAL:
                        rfpath_meshes = [Path('PAM/object_meshes').joinpath(obj['meta']['oid'], Path(obj['meta']['instance_path']).name) for obj in meta['objects'].values()]
                    else:
                        rfpath_meshes = [obj['meta']['instance_path'] for obj in meta['objects'].values()]
                    ...
                    objs_scale = torch.FloatTensor([obj['meta']['scale'] for obj in meta['objects'].values()])
                    ...
                    world_tform4x4_objs = tform4x4_broadcast(world_tform4x4_cam[None, ], cam_tform4x4_objs)
                    ...
                    categories = [obj['meta']['class_name'] for obj in meta['objects'].values()]
                    ...
                    l_objs_name = [obj['meta']['oid'] for obj in meta['objects'].values()]
                    l_objs_valid = torch.BoolTensor([obj['is_valid'] for obj in meta['objects'].values()]).tolist()



                    from od3d.cv.visual.show import show_scene, show_img, render_trimesh_to_tensor, show_imgs
                    from od3d.cv.geometry.objects3d.meshes.meshes import Meshes, PROJECT_MODALITIES, VERT_MODALITIES
                    #from od3d.cv.io import read_image
                    ...
                    if frame_meta.get_fpath(path_meta=path_meta).exists() and not config.extract_meta.override:
                        logger.info(f"already extracted frame {frame_id} in scene {scenes_names[subset][s]} in subset {subset}")
                    else:
                        frame_meta.save(path_meta=path_meta)

            if subset not in [OMNI6DPOSE_SUBSETS.TEST_REAL, OMNI6DPOSE_SUBSETS.TEST_IKEA,
                              OMNI6DPOSE_SUBSETS.TEST_MATTERPORT3D, OMNI6DPOSE_SUBSETS.TEST_SCANNETPP,
                              OMNI6DPOSE_SUBSETS.TRAIN_IKEA, OMNI6DPOSE_SUBSETS.TRAIN_MATTERPORT3D,
                ...
                # what about category
                categories = Omni6DPose.get_class_specific_categories(config.get("categories", None))
                import re
                match = re.match(r"([a-z]+)_([a-z]+)_([a-z]+)_([a-z0-9]+)_([a-z]+)([0-9]+)_theta_([a-z]+)([0-9]+)", subset, re.I)
                if match and len(match.groups()) == 8:
                    train_or_test, real_or_syn, render_type, viewpoints_type, viewpoints_sample_type, viewpoints_count, theta_sample_type, theta_count = match.groups()
                ...
                if real_or_syn == "real":
                    if train_or_test == "train":
                        ...
                        raise Exception(msg)
                    # ['class_list']['instance_dict']
                    assets_meta = read_json(path_raw.joinpath("Meta/real_obj_meta.json"))['instance_dict']
                else:
                    # ['class_list']['instance_dict']
                    assets_meta = read_json(path_raw.joinpath("Meta/obj_meta.json"))['instance_dict']
                    assets_meta = {key: val for key, val in assets_meta.items() if val['tag']['datatype'] == train_or_test}
                ...
                for category in categories:
                    # filter category
                    assets_meta_cat = {key: val for key, val in assets_meta.items() if val['class_name'] == category}
                    ...
                    if sequences_count_max_per_category is not None:
                        assets_meta_cat = {k: assets_meta_cat[k] for k in list(assets_meta_cat)[:sequences_count_max_per_category]}
                    ...
                    for asset_name, asset_data in assets_meta_cat.items():
                        ...
                        if sequence_meta.get_fpath(path_meta=path_meta).exists() and not config.extract_meta.override:
                        ...
                        fpath_mesh = path_raw.joinpath('PAM/object_meshes/' + asset_data['object_id'] + '/' + Path(asset_data['obj_path']).name)
                        ...
                        if real_or_syn == "real":
                            ...
                            _mesh_trimesh_verts = torch.Tensor(_mesh_trimesh.vertices)
                            obj_scale = max(_mesh_trimesh_verts.max(dim=0)[0] - _mesh_trimesh_verts.min(dim=0)[0])
                        else:
                        ...
                        from od3d.cv.io import write_image, write_mask_image, write_depth_image, \
                        ...
                        if viewpoints_type.startswith('icotraj') or viewpoints_type.startswith('icorealtraj'):
                            ...
                            traj_length = float(viewpoints_type.removeprefix(prefix)) / 100 * math.pi * 2
                            cams_tform4x4_obj = get_ico_traj_cam_tform4x4_obj_for_viewpoints_count(
                                viewpoints_count=viewpoints_count,
                                radius=4. * obj_scale,
                                theta_count=theta_count, geodesic_distance=traj_length, real=real).to(device)
                        else:
                            cams_tform4x4_obj = get_ico_cam_tform4x4_obj_for_viewpoints_count(
                                viewpoints_count=viewpoints_count, radius=4. * obj_scale, theta_count=theta_count,
                                viewpoints_uniform=viewpoints_sample_uni, theta_uniform=theta_sample_uni).to(device)
                        ...
                        cams_tform4x4_obj = tform4x4_broadcast(cams_tform4x4_obj, OBJ_TFORM_OBJ_SHAPENET.to(device))
                        cam_intr4x4 = get_default_camera_intrinsics_from_img_size(W=W, H=H).to(device)
                        try:
                            mesh = Meshes.read_from_ply_file(fpath=frame_fpath_mesh, device=device, load_texts=True)
                        except Exception:
                            # print(frame_fpath_mesh)
                            logger.warning(f'could not load texture {frame_fpath_mesh}')
                            continue

                        logger.info(f"sequence {sequence_name}, {len(mesh.verts)} vertices")
                        ...
                        for i, cam_tform4x4_obj in enumerate(cams_tform4x4_obj):
                            ...
                            l_objs_valid = torch.ones((1,)).to(dtype=torch.bool).tolist()
                            ...
                            frame_meta = Omni6DPose_FrameMeta(
                                ...
                                rfpath_pxl_cat_id=frame_fpath_pxl_cat_id.relative_to(path_raw),
