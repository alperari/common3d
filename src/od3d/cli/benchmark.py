...
import math
from od3d.cli._platform import get_jobs_names
...
def run(
    ...
    max_count_sleep_time = max_count_sleep_time_in_mins * 60  # 10 mins
    ...
    while len(methods_cfgs) > 0:
        ...
        if platform_base == "slurm":
            ...
            slurm_failed_hosts_fpath = Path("/tmp/slurm_failed_hosts.yaml")
            if slurm_failed_hosts_fpath.exists():
                dict_slurm_failed_hosts = read_dict_from_yaml(slurm_failed_hosts_fpath)
                dt_last = dt_from_str(dict_slurm_failed_hosts["time"])
                dt_now = datetime.datetime.now()
                if td_to_mins(dt_now - dt_last) < 30:
                    logger.info(f"td: {td_to_mins(dt_now - dt_last)} mins")
                    slurm_failed_hosts = dict_slurm_failed_hosts["hosts"]
                    logger.info(f"using stored slurm failed nodes {slurm_failed_hosts}")

            if slurm_failed_hosts is None:
                slurm_failed_runs = get_runs_multiple(
                    # benchmark=benchmark,
                    # ablation=ablation,
        ...
        for i, method_cfg in tqdm(enumerate(methods_cfgs)):
            ...
            method_cfg.run_name = run_name
            if slurm_failed_hosts is not None:
                with open_dict(method_cfg):
                    if len(slurm_failed_hosts) > 0:
                        method_cfg.platform.nodes_exclude = ",".join(slurm_failed_hosts)
                    else:
                        method_cfg.platform.nodes_exclude = None
