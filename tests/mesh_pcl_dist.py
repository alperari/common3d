...
faces_normals = torch.cross(verts[faces[:, 1]] - verts[faces[:, 0]], verts[faces[:, 2]] - verts[faces[:, 0]], dim=-1)
...
def get_pts_face_dist(pts3d, verts, faces):
    ...
    pts3d_on_edge01[(pts3d_signed_dist_to_edge01 > 1.) + (pts3d_signed_dist_to_edge01 < 0.)] = -torch.inf
    pts3d_on_edge02[(pts3d_signed_dist_to_edge02 > 1.) + (pts3d_signed_dist_to_edge02 < 0.)] = -torch.inf
    pts3d_on_edge12[(pts3d_signed_dist_to_edge12 > 1.) + (pts3d_signed_dist_to_edge12 < 0.)] = -torch.inf
    ...
    pts3d_on_face_closest_ids = (pts3d[:, None] - pts3d_on_faces_closest).norm(dim=-1).min(dim=-1).indices
    pts3d_on_face_closest = batched_index_select(pts3d_on_faces_closest, index=pts3d_on_face_closest_ids[:, None], dim=1)[:, 0]
    pts3d_on_face_closest_inside = batched_index_select(pts3d_on_faces_closest_inside, index=pts3d_on_face_closest_ids[:, None], dim=1)[:, 0]
