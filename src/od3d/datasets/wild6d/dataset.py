...
from od3d.datasets.wild6d.frame import WILD6D_Frame, WILD6D_FrameMeta
from od3d.datasets.wild6d.sequence import WILD6D_Sequence, WILD6D_SequenceMeta
...
class WILD6D(OD3D_SequenceDataset):
    all_categories = list(WILD6D_CATEGORIES)
    map_od3d_categories = MAP_CATEGORIES_OD3D_TO_WILD6D
    sequence_type = WILD6D_Sequence
    frame_type = WILD6D_Frame
    ...
    def __init__(
        ...
        categories: List[WILD6D_CATEGORIES] = None,
    ...
    def extract_meta(config: DictConfig):
        ...
        path_meta = WILD6D.get_path_meta(config=config)
        ...
        if dict_nested_frames is not None:
            categories = list(dict_nested_frames.keys())
        elif config.get("categories", None) is not None:
            categories = config.get("categories")
        else:
            categories = WILD6D_CATEGORIES.list()
        ...
        for category in categories:
            ...
            for sequence_name in sequences_names:
                ...
                sequence_meta = WILD6D_SequenceMeta(
                    category=category,
                    name=sequence_name,
                )
