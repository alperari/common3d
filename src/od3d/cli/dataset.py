...
def visualize_selfsup(
    ...
    model_name: str = typer.Option("dinov2_sph_vitb14_frozen_base", "-m", "--model"),
    ...
    if model_use_feat_up:
        ...
        res_high = multiplicator * 14
        res_low = res_high + multiplicator * 2

        model_transform = SequentialTransform(
            [
                RGB_UInt8ToFloat(),
        ...
        model_out_dim = model.out_dim
        res_high = res_low * model.downsample_rate
    ...
    if transform_name != "None":  # centerzoom512
        ...
        dataset.transform = model_transform
    ...
    for batch in iter(dataloader):
        ...
        pc_feats_mean = _pca_feats.mean(dim=0)
        featmap_pca = featmap - pc_feats_mean[:, None, None]
        featmap_pca = (
            torch.mm(
    ...
    logger.info(pca_S)
    pc_feats_mean = pca_feats.mean(dim=0)

    for b, batch in enumerate(iter(dataloader)):
        logger.info(f"{batch.name_unique[0]}")  # sequence_name[0]}')
        batch.to(device=device)
        ...
        if center:
            feats = feats - pc_feats_mean[:, None, None]
        feats_pca = (
            torch.mm(
                feats.permute(0, 2, 3, 1).reshape(-1, model_out_dim),
                pca_V[:, 0:pca_dim],
            )
...
def visualize_meshes_per_category(
    ...
    dataset.visualize_meshes_per_category(
        viewpoints_count=viewpoints_count,
        H=height,
        W=width,
...
def rsync_raw(
    ...
    paths_source = Path(config_source.dataset.path_raw).joinpath(rpath)
    paths_target = Path(config_target.dataset.path_raw).joinpath(rpath)
...
def visualize(
    ...
    if transform_name != "None":
        ...
        dataset.transform.H = 512
        dataset.transform.W = 512
