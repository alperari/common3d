...
_MAX_PIXEL_ERROR = 20.0
_MAX_AZIMUTH_ERROR = 30.0
_MAX_POLAR_ERROR = 20.0
...
class Evaluator:
    ...
    def __init__(self, height=640, width=480):
        ...
        self._iou_3d = 0.0
        self._azimuth_error = 0.0
        self._polar_error = 0.0

        self._iou_thresholds = np.linspace(0.0, 1.0, num=_NUM_BINS)
    ...
    def evaluate(self, batch):
        ...
        for boxes, label, plane in zip(results, labels, planes):
            ...
            for instance, instance_3d, visibility in zip(
                ...
                if (
                    visibility > self._vis_thresh
                    and self._is_visible(instance[0])
                    and instance_3d[0, 2] < 0
                ):
                    num_instances += 1
    ...
    def evaluate_rotation(self, box, instance):
        ...
        abs_error_angles = np.minimum(
            abs_error_angles,
            np.absolute(math.pi * np.ones(3) - abs_error_angles),
        )
        ...
        angular_distance = math.acos((rotation_error_trace - 1.0) / 2.0)
